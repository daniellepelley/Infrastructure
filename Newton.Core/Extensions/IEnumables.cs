...
namespace Newton.Extensions
{
    ...
    public static class IEnumerables
    {
        ...
        /// <summary>
        /// Returns a collection of the difference between the values in the collection and the limit
        /// </summary>
        public static IEnumerable<double> Difference(this IEnumerable<double> source, double limit)
        {
            return source.Select(s => (limit > s ? limit - s : 0));
        }

        /// <summary>
        /// Returns a collection of the difference between the sum of elements in each position across the collections and the limit
        /// </summary>
        /// <remarks>
        /// One use is with utilisation and efficiency figures
        /// </remarks>
        public static IEnumerable<double> Difference(this IEnumerable<IEnumerable<double>> source, double limit)
        {
            foreach (var p in source.Pivot())
            {
                yield return limit - p.Sum();
            }
        }
        ...
        //TODO: Pivot Extensions to be tested
        /// <summary>
        /// Pivots a collection of IEnumerable of type T
        /// </summary>
        public static IEnumerable<IEnumerable<T>> Pivot<T>(this IEnumerable<IEnumerable<T>> source)
        {
            ...
            int count = source.Select(s => s != null ? s.Count() : 0).Max();
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                foreach (var item in source)
                {
                    if (item != null &&
                        item.Count() > i)
                    {
                        newItem.Add(item.Skip(i).Take(1).FirstOrDefault());
                    }
                }
                list.Add(newItem);
            }
